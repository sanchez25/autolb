  ...
  <v-card>
    ...
    <v-card-actions class="justify-end">
      ...
      <v-btn
          color="blue darken-1"
          text
          @click="downloadContent(subproject_name)"
      >
        Отправить
      </v-btn>
    </v-card-actions>
...
export default {
  ...
  methods: {
    async downloadContent(subproject_url) {
      try {
        this.dialogProgress = true
        let formData = new FormData();
        formData.append("file", document.getElementById('file').files[0]);
        formData.append("subproject", `${subproject_url}`);
        const response = await this.$axios.post('/frontend/api/project/download_content', formData, {
          headers: {
